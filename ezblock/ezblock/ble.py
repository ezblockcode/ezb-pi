...
class BLE(_Basic_class):
    def __init__(self,name=LOCAL_NAME):
        ...
        if not self.is_ble_only():
            ...
            run_command("sudo btmgmt power off")
            run_command("sudo btmgmt le on")
            run_command("sudo btmgmt bredr off")
            run_command("sudo btmgmt power on")
            time.sleep(0.5)
            self.is_ble_only()
    ...
    def is_ble_only(self):
        settings = []
        _, result = run_command("sudo btmgmt info")
